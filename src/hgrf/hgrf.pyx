...
cdef extern from "model_general_xy.c":
    enum: NSTENCIL
...
cdef extern from "main.c":
    ...
    int c_main(int nk, int ni, int nj, int solver_id, int maxiter, int verbose, double param_r12,
               double* source, double* spatial_angle_image, double* velocity, double* correlation_time_image,
               double* correlation_length_image, double* output_video, double* values)
...
def run(nt, nx, ny, solver_id, maxiter, verbose, param_r12,
        np.ndarray[double, ndim=3, mode="c"] source,
        np.ndarray[double, ndim=2, mode="c"] spatial_angle_image,
        np.ndarray[double, ndim=3, mode="c"] velocity,
    ...
    cdef np.ndarray[double, ndim=1, mode="c"] values = np.zeros(shape=(NSTENCIL*nt*nx*ny), dtype=np.float64)
    c_main(nt, nx, ny, solver_id, maxiter, verbose, param_r12, &source[0,0,0],
           &spatial_angle_image[0,0], &velocity[0,0,0], &correlation_time_image[0,0],
           &correlation_length_image[0,0], &output_video[0,0,0], &values[0])
