...
cdef extern from "evolve.c":
    ...
    void get_angle_derivative_image(double* gradient, double PARAM_RAT, double* principal_angle_image,
                             double* diffusion_coefficient_image, double* advection_velocity_image,
                             double* correlation_time_image, double* forward, double* adjoint)
...
def adjoint_angle_derivative(PARAM_RAT,
                  np.ndarray[double, ndim=2, mode="c"] c_principal_angle_image,
                  np.ndarray[double, ndim=2, mode="c"] c_diffusion_coefficient_image,
                  np.ndarray[double, ndim=3, mode="c"] c_advection_velocity_image,
                  np.ndarray[double, ndim=2, mode="c"] c_correlation_time_image,
                  np.ndarray[double, ndim=3, mode="c"] forward,
                  np.ndarray[double, ndim=3, mode="c"] adjoint):
    """TODO"""
    cdef np.ndarray[double, ndim=2, mode="c"] gradient = np.zeros(shape=(N, N), dtype=np.float64)
    get_angle_derivative_image(&gradient[0,0], PARAM_RAT, &c_principal_angle_image[0,0], &c_diffusion_coefficient_image[0,0],
                        &c_advection_velocity_image[0,0,0], &c_correlation_time_image[0,0], &forward[0,0,0], &adjoint[0,0,0])
