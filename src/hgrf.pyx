...
cdef extern from "general_xy_matrices.c":
    ...
    void get_velocity_image(int ni, int nj, double* spatio_temporal_angle, double direction, double param_rct, double opening_angle)
...
def get_generalxy_velocity(nx, ny, direction, param_rct, opening_angle):
    """TODO"""
    cdef np.ndarray[double, ndim=3, mode="c"] velocity = np.zeros(shape=(nx, ny, 2), dtype=np.float64)
    get_velocity_image(nx, ny, &velocity[0,0,0], direction, param_rct, opening_angle)
