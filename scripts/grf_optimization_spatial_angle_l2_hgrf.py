...
run_name =  'l2_spatial_angle_init[zero]_seed[{seed}]'.format(seed=solver.seed) if args.run_name is None else args.run_name
...
forward_fn = lambda: solver.run(maxiter=args.maxiter, num_frames=args.nt, verbose=False, n_jobs=args.n_jobs)
adjoint_fn = lambda source: solver.run(source=source, maxiter=args.maxiter, verbose=False, n_jobs=args.n_jobs)
gradient_fn = lambda forward, adjoint: solver.get_spatial_angle_gradient(forward, adjoint)
...
options={'maxiter': 1000, 'disp': True, 'initial_step_size': 1.0, 'maxls': 10}
optimizer = Optimizer(objective_fn, callback_fn=callback_fn, options=options, method='GD')

# options={'maxiter': 1000, 'disp': True, 'ftol': 1e-6}
# optimizer = Optimizer(objective_fn, callback_fn=callback_fn, options=options, method='L-BFGS-B')
